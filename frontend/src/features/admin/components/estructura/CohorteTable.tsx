...
interface CohorteTableProps {
  ...
  onToggleEstado: (item: Cohorte) => void;
}

export default function CohorteTable({ items, onEdit, onToggleEstado }: CohorteTableProps) {
  return (
      ...
      <table className="w-full text-sm">
        <thead className="bg-muted">
          <tr>
            ...
            <th className="px-4 py-3 text-left font-medium text-muted-foreground">Carrera</th>
        ...
        <tbody className="divide-y divide-border">
          {items.map((item) => (
            <tr key={item.id} className="hover:bg-muted/50">
              ...
              <td className="px-4 py-3">{item.anio}</td>
              <td className="px-4 py-3 text-muted-foreground">{item.carrera_nombre ?? '-'}</td>
              <td className="px-4 py-3 text-muted-foreground">
                {item.vigencia_desde} → {item.vigencia_hasta}
              </td>
