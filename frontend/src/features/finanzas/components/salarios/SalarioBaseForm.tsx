...
import type { SalarioBase, SalarioBaseCreate } from '../../types/salarios.types';
...
interface SalarioBaseFormProps {
  ...
  onSubmit: (data: SalarioBaseCreate) => void;
  onCancel: () => void;
  isLoading?: boolean;
}

export default function SalarioBaseForm({ item, onSubmit, onCancel, isLoading }: SalarioBaseFormProps) {
  const {
    register,
    handleSubmit,
    reset,
  ...
  const handleFormSubmit = (data: FormData) => {
    onSubmit(data as SalarioBaseCreate);
  };

  return (
    <Card>
      <CardHeader>
        <CardTitle className="text-base">{item ? 'Editar salario base' : 'Nuevo salario base'}</CardTitle>
      </CardHeader>
      <CardContent>
