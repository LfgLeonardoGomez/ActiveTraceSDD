"""012_aviso_acknowledgment

C-15: Tablas de avisos y acknowledgment.

Schema changes:
- aviso: anuncios dirigidos por alcance, severidad, vigencia temporal.
- acknowledgment_aviso: confirmación de lectura por usuario.
- permisos RBAC: avisos:publicar, avisos:confirmar (ya seedeados en 002).
"""
...
revision: str = "012_aviso_acknowledgment"
down_revision: Union[str, Sequence[str], None] = "011_avisos"
...
def upgrade() -> None:
    ...
    op.create_index("ix_ack_aviso_aviso", "acknowledgment_aviso", ["aviso_id"])
    op.create_index("ix_ack_aviso_usuario", "acknowledgment_aviso", ["usuario_id"])


def downgrade() -> None:
    op.drop_index("ix_ack_aviso_usuario", table_name="acknowledgment_aviso")
    op.drop_index("ix_ack_aviso_aviso", table_name="acknowledgment_aviso")
    op.drop_constraint("uq_ack_aviso_usuario", "acknowledgment_aviso", type_="unique")
    op.drop_table("acknowledgment_aviso")

    op.drop_index("ix_aviso_alcance", table_name="aviso")
    op.drop_index("ix_aviso_tenant_activo_vigencia", table_name="aviso")
    op.drop_index("ix_aviso_tenant", table_name="aviso")
    op.drop_table("aviso")
